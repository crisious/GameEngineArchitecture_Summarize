...
Houndi 같은 전문 사용 툴로도 만들지만, 너무 퀄리티가 크면 게임 엔진에서 소화하기 힘들다.

엔진에서 처리할 만한 수준의 효과를 구현하는 도구를 만들어 사용한다. \(게임에서 보이는 상황을 확인하기 쉽다.\)

### 1.7.7 게임 월드 데이터 / 월드 데이터

아직 게임 월드 에디터에 상용적 툴은 없지만, 많은 상용 게임 엔진에서 월드 데이터를 구현한다.

* Radiant : Quake 기반 게임 엔진 에디터
* Hammer : 하프 라이프 2 기반 Source 게임 엔진 사용 에디터
* UnrealEd : 언리얼 엔진 에디터. 자원 관리 도구 역할 같이 수행.
...
![](/assets/1_34.PNG)
 ...
 언리얼 엔진의 경우 게임 엔진과 완전히 통합된 형태

 툴이 게임 엔진의 자료 구조 접근에 제약이 없고 툴과 게임 엔진이 같은 자료 구조를 서로 다른 형태로 갖고 있을 필요가 없다.

 그리고 에디터에서 게임을 실행할 때 빠르다.

 게임 엔진에 문제가 생긴 경우 데이터도 같이 사용 할 수 없어 작업이 중지될 수 있다.

 엔진과 자원 생성 툴이 너무 밀접하게 연결되 있으면 생산성을 떨어뜨릴 수 있다.
